...
class GroupServiceClass {
    /**
     * Create a new group with a random passcode, and add the creator as
     * the first member. Retries on passcode collision — rare given 6 digits
     * of entropy, but the DB's unique constraint makes it possible.
     */
  async createGroup(userId: string, name?: string, displayName?: string): Promise<Group | null> {
      ...
      for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
          ...
          if (error) {
                    if (error.code === '23505') {
                                // Unique constraint violation on passcode — try a new one
  ...
  /**
     * Join an existing group by its 6-digit passcode. Fails if the group
     * doesn't exist, has expired, or is already at MAX_MEMBERS.
     */
