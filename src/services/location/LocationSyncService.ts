...
class LocationSyncServiceClass {
  ...
  /**
       ...
       *
       * userId MUST be a real auth.uid() from AuthService.initialize() — not
       * an arbitrary string — since RLS policies check it against the
       * authenticated session on the shared Supabase client.
       */
  async startSync(
          groupId: string,
          userId: string,
